...
Coarse time synchronization between core AS’s control services and SCION routers is necessary because path segments are generated by core ASes control service instances and later used to construct data plane paths. Specifically, routers use the timestamp in the Info Field and the expiration time of Hop Fields to check Hop Field validity, see [](#process-router-ingress).

A segment's originating core AS control service and the routers that the segment refers to all have different clocks. Their differences affect the validation process:
...
The minimal expiration time for a Hop Field is 337.5 seconds (see [](#hopfld)). Clock offsets between a router and core ASes in the order of minutes can be therefore tolerated.
...
Security considerations related to this issue are discussed in {{I-D.dekater-scion-controlplane}}. Considerations on clock inaccuracies and beaconing are discussed in the homonymous section in {{I-D.dekater-scion-controlplane}}.
